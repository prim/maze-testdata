...
内存布局预期 (Python 2.7):
    - <class A> instance:      ~352 bytes (对象头 + __dict__)
    - {"a", "b"} 外层 dict:     ~288 bytes
    - {int => int}<256:        ~12584 bytes (内层大字典)
    
合并模式预期:
    - <class A> instance avg_size 应约等于 352 + 288 + 12584 ≈ 13224 bytes
"""
...
# =========================================================
# 预期值常量 (基于 Python 2.7 测量)
# =========================================================
EXPECTED_COUNT = 10000

# 普通模式下各组件的独立大小
NORMAL_CLASS_A_AVG_SIZE = 352          # class A 实例本身 (包含 __dict__ 指针)
NORMAL_OUTER_DICT_AVG_SIZE = 288       # {"a", "b"}
NORMAL_INNER_DICT_AVG_SIZE = 12584     # {int => int}<256
...
def validate_normal_mode(items, summary):
    ...
    if class_a:
        ...
        # 普通模式下 avg_size 应该较小（只有对象头）
        # Python 2.7 的实例对象较大，约 352 bytes
        if avg_size < 500:  # 合理范围
            print("  ✓ avg_size reasonable for normal mode (< 500)")
        else:
    ...
    # =========================================================
    print("\n[Check 2] Inner dict {int => int}<256...")
    inner_dict = find_type_containing(items, "{int => int}")
    if inner_dict:
...
def validate_merge_mode(items, summary):
    ...
    # =========================================================
    print("\n[Check 2] Inner dict {int => int}<256 (should be merged)...")
    inner_dict = find_type_containing(items, "{int => int}")
    if inner_dict:
        amount = inner_dict["amount"]
...
def validate_merge_comparison(normal_data, merge_data):
    ...
    # =========================================================
    print("\n[Inner dict {int => int} comparison]")
    normal_inner = find_type_containing(normal_items, "int => int")
    merge_inner = find_type_containing(merge_items, "int => int")
