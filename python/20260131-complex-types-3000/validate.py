...
# -*- coding: utf-8 -*-
"""
验证脚本 - 大规模复杂类型测试用例 (每种对象 3000 个)

验证 maze 分析结果是否正确识别了各种 Python 类型
基于 20260129-complex-types-311/validate.py 模板
"""
...
def validate(data):
    ...
    # 验证 pymempool 对象数量 (大规模测试应该 > 100000)
    pymempool_objects = summary.get("pymempool_objects", 0)
    print("  pymempool_objects = %d" % pymempool_objects)
    assert pymempool_objects > 100000, "Expected > 100000 pymempool objects, got %d" % pymempool_objects
    print("  ✓ pymempool_objects > 100000")
    ...
    assert person["amount"] == 3000, "Expected 3000 PersonClass, got %d" % person["amount"]
    print("  ✓ PersonClass amount = 3000")
    
    # GameEntity - 应该有 3000 个
    ...
    assert game_entity["amount"] == 3000, "Expected 3000 GameEntity, got %d" % game_entity["amount"]
    print("  ✓ GameEntity amount = 3000")
    ...
    assert tree_node["amount"] == 3000, "Expected 3000 TreeNode, got %d" % tree_node["amount"]
    print("  ✓ TreeNode amount = 3000")
    
    # SimpleClass - 应该有 3000 个
    ...
    assert simple_class["amount"] == 3000, "Expected 3000 SimpleClass, got %d" % simple_class["amount"]
    print("  ✓ SimpleClass amount = 3000")
    
    # ============================================================
    # 验证 dataclass 类型 (Python 3.7+) - 每种 3000 个
    # ============================================================
    print("\nValidating dataclass instances:")
    
    # Player dataclass - 应该有 3000 个
    ...
    assert player["amount"] == 3000, "Expected 3000 Player, got %d" % player["amount"]
    print("  ✓ Player dataclass amount = 3000")
    
    # Config frozen dataclass - 应该有 3000 个
    ...
    assert config["amount"] == 3000, "Expected 3000 Config, got %d" % config["amount"]
    print("  ✓ Config dataclass amount = 3000")
    ...
    assert point["amount"] == 1200, "Expected 1200 Point, got %d" % point["amount"]
    print("  ✓ Point amount = 1200")
    ...
    if rectangle is not None:
        ...
        assert rectangle["amount"] == 600, "Expected 600 Rectangle, got %d" % rectangle["amount"]
        print("  ✓ Rectangle amount = 600")
    else:
    ...
    assert deque_type["amount"] == 1200, "Expected 1200 deque, got %d" % deque_type["amount"]
    print("  ✓ deque amount = 1200")
    ...
    assert total_sets >= 2300, "Expected >= 2300 set objects, got %d" % total_sets
    print("  ✓ Total set objects >= 2300")
