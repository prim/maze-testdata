...
void thread_worker(int thread_id, std::vector<AllocTask> tasks)
{
	...
	printf("  Thread %d: completed, waiting for gcore...\n", thread_id);
	g_threads_done++;

	// 非主线程等待 gcore 完成，避免线程退出导致 mimalloc abandon segment
	if (thread_id != 0) {
		while (!g_gcore_done.load()) {
			sleep(1);
		}
	}
}
...
int main()
{
	...
	// detach 线程，让它们在 gcore 期间保持存活
	for (auto &t : threads)
		t.detach();

	thread_worker(0, thread_tasks[0]);

	// 等待所有线程完成分配（但线程不会退出，在等 g_gcore_done）
