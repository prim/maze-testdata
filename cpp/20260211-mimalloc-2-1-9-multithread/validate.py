...
def validate(data):
    ...
    # 要求：不低于目标数量，且不超过目标数量的 1%
    large_expected = [
        ("1MB", 1048576, 1048576 * 1.5, 100),
        ("2MB", 2097152, 2097152 * 1.5, 100),
        ("3MB", 3145728, 3145728 * 1.5, 100),
    ]

    all_passed = True

    for size, expected_amount in small_expected.items():
        actual = size_to_amount.get(size, 0)
        max_expected = int(expected_amount * 1.01)
        passed = expected_amount <= actual <= max_expected
        if not passed:
            all_passed = False
        status = "PASS" if passed else "FAIL"
        print(
            f"  {size:>10} bytes: {actual:>6} / {expected_amount:>6} (max: {max_expected}) {status}"
        )

    for label, lo, hi, expected_amount in large_expected:
        ...
        status = "PASS" if passed else "FAIL"
        print(
            f"  {size_info:>10} bytes: {actual:>6} / {expected_amount:>6} (max: {max_expected}) {status}  [{label}]"
        )
